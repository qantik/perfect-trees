...
foreach f (10 100 1000 5000)
  ...
  foreach r (`seq 384 384`)
    echo '=============================== '$r
  
    sed -i 's/.*generic (r .*/    generic (r : integer := '$r');/' ../triva.vhd
    sed -i 's/.*constant r .*/    constant r  : integer := '$r';/' ../triva_tb.vhd
    ...
    echo $r' '$area' '$pic' '$sec' '$pow' '$ene >> trivia-$f-ns.dat
  end
