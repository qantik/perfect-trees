...
foreach f (10 100 1000 5000)
  ...
  sed -i 's/.*constant clk_period .*/    constant clk_period   : time := '$f' ns;/' ../trivium_tb.vhd

  foreach r (`seq 288 288`)
    echo '=============================== '$r
  
    sed -i 's/.*generic (r .*/    generic (r : integer := '$r');/' ../trivium.vhd
    sed -i 's/.*constant r .*/    constant r  : integer := '$r';/' ../trivium_tb.vhd
    ...
    echo $r' '$area' '$pic' '$sec' '$pow' '$ene >> trivium-$f-ns.dat
  end
