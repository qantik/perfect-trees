...
foreach f (10 100 1000 5000)
  ...
  foreach r (`seq 256 256`)
    echo '=============================== '$r
  
    sed -i 's/.*generic (r .*/    generic (r : integer := '$r');/' ../kreyvium.vhd
    sed -i 's/.*constant r .*/    constant r  : integer := '$r';/' ../kreyvium_tb.vhd
    ...
    echo $r' '$area' '$pic' '$sec' '$pow' '$ene >> kreyvium-$f-ns.dat
  end
