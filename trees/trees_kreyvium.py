...
def tree_build_ref():
    """Build Kreyvium equation tree.
    x65 + x92 + (x90 * x91) + x169 + iv_r
    x161 + x176 + (x174 * x175) + x263 
    x242 + x287 + (x285 * x286) + x68 + k_r
    """

    G = nx.DiGraph()
    x = ['x_%d' % i for i in range(0, 288)]
    k = ['k_%d' % i for i in range(0, 128)]
    iv =['iv_%d' % i for i in range(0, 128)]
    ...
    for i in range(0, r): 
        ...
        G.add_edges_from([(z1, x[65]), (z1, x[92]), (z1, x[90]), (z1, x[91]), (z1, x[170]), (z1, iv[i % 128])])
        G.add_edges_from([(z2, x[161]), (z2, x[176]), (z2, x[174]), (z2, x[175]), (z2, x[263])])
        G.add_edges_from([(z3, x[242]), (z3, x[287]), (z3, x[285]), (z3, x[286]), (z3, x[68]), (z3, k[i % 128])])
...
def tree_build_alt(a1=66, b1=69, c1=66, fa=78, fb=87, fc=69, a2=93, b2=84, c2=111):
    """Build Kreyvium equation tree.
    x1_65 + x1_92 + (x1_90 * x1_91) + x2_77 + iv_r
    x2_68 + x2_83 + (x2_81 * x2_82) + x3_86 
    x3_65 + x3_110 + (x3_108 * x3_109) + x1_68 + k_r
    """
    ...
    for i in range(0, r): 
        ...
        G.add_edges_from([(z1, x1[a1-1]), (z1, x1[a2-1]), (z1, x1[a2-3]), (z1, x1[a2-2]), (z1, x2[fa-1]), (z1, iv[i % 128])])
        G.add_edges_from([(z2, x2[b1-1]), (z2, x2[b2-1]), (z2, x2[b2-3]), (z2, x2[b2-2]), (z2, x3[fb-1])])
        G.add_edges_from([(z3, x3[c1-1]), (z3, x3[c2-1]), (z3, x3[c2-3]), (z3, x3[c2-2]), (z3, x1[fc-1]), (z3, k[i % 128])])
            ...
            for z in range(-31, 9):
                if (x + y + z) == 0 and x % 3 == 0 and y % 3 == 0 and z % 3 == 0:
                    ...
                    for i in range(30000):
                        a1 = random.randint(60, a2-4)
                        b1 = random.randint(60, b2-4)
                        c1 = random.randint(60, c2-4)
                        ...
                        if t[o//100] < 30 and a1 % 3 == 0 and b1 % 3 == 0 and c1 % 3 == 0 and fa % 3 == 0 and fb % 3 == 0 and fc % 3 == 0:
