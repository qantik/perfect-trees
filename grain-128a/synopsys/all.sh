...
foreach f (100 1000 5000)
  ...
  foreach r (`seq 1 2 256`)
    echo '=============================== '$r
  
    sed -i 's/.*generic (r .*/    generic (r : integer := '$r');/' ../grain_128a.vhd
    sed -i 's/.*constant r .*/    constant r  : integer := '$r';/' ../grain_128a_tb.vhd
    ...
    echo $r' '$pic' '$sec' '$pow' '$ene >> grain128a-$f-ns.dat
  end
